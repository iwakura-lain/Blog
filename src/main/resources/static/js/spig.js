...
jQuery(document).ready(function ($) {
    $("#spig").mousedown(function (e) {
        if(e.which==3){
        showMessage("秘密通道:<br />    <a href=\"#\" title=\"首页\">首页</a>    <a href=\"#\" title=\"项目\">项目</a>    <a href=\"#\" title=\"后台\">后台</a> ",10000);
}
});
...
jQuery(document).ready(function ($) {
    if (isindex) { //如果是主页
        ...
        showMessage('欢迎' + visitor + '来到antigenMHCのBlog ' + title + ' ', 6000);
    }
    $(".spig").animate({
...
jQuery(document).ready(function ($) {
    ...
    $(".mumu").click(function () {
        if (!ismove) {
            stat_click++;
            if (stat_click > 4) {
                msgs = ["你有完没完呀？", "你摸你🐎呢" + stat_click + "次了", "非礼呀！救命啊！大师兄"];
                var i = Math.floor(Math.random() * msgs.length);
                //showMessage(msgs[i]);
            } else {
